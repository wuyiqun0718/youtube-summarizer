...
export default function TranscriptPanel({ captions, videoId }: TranscriptPanelProps) {
  ...
  return (
    <div className="mt-6">
      ...
      {open && (
        <div className="mt-4 bg-zinc-900/50 rounded-xl border border-zinc-800 overflow-hidden">
          {/* Toggle bar */}
          ...
          <div className="max-h-[500px] overflow-y-auto p-4">
            {plainText ? (
              <p className="text-sm text-zinc-400 leading-7 whitespace-pre-wrap select-text">
                ...
                {captions.map((seg, i) => (
                  <div key={i} className="flex items-start gap-1 group">
                    <button
                      ...
                      className="flex-1 flex items-start gap-3 px-3 py-2 rounded-lg hover:bg-zinc-800/50 transition-all text-left"
                    >
                      ...
                      <span className="text-sm text-zinc-400 leading-6 group-hover:text-zinc-200 transition-colors">
                        {seg.text}
                      </span>
                    </button>
                    {videoId && (
                      <a
                        ...
                        className="mt-2.5 shrink-0 text-zinc-700 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
