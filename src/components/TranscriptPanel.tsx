...
export default function TranscriptPanel({ captions, videoId }: TranscriptPanelProps) {
  ...
  return (
    <div>
      ...
      {/* Content */}
      {plainText ? (
        <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-7 whitespace-pre-wrap select-text">
          ...
          {captions.map((seg, i) => (
            <div key={i} className="flex items-start gap-1 group">
              ...
              >
                ...
                <span className="text-sm text-zinc-500 dark:text-zinc-400 leading-6 group-hover:text-zinc-900 dark:group-hover:text-zinc-200 transition-colors">
                  {seg.text}
                </span>
              </button>
              {videoId && (
                <a
                  ...
                  className="mt-2.5 shrink-0 text-zinc-300 dark:text-zinc-700 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
